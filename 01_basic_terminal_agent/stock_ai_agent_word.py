...
使用方法：
   （只要使用命令运行，而且只运行一次） python stock_ai_agent_word.py 000001 -q "请分析这只股票的近期走势"
...
def main():
    """主函数：处理命令行交互"""
    parser = argparse.ArgumentParser(description="股票分析AI智能体（带Word报告生成）")
    parser.add_argument("symbol", help="股票代码（如：000001、sz000001）")
